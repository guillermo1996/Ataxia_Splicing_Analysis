...
juctionPairing <- function(metadata,
  ...
  if (rw_disk & !overwrite & file.exists(paste0(output_path, "all_distances_raw.rds"))) {
    logger::log_info("\t\t Ignoring the pairing process.") 
    logger::log_info("\t\t File ", output_path, "all_distances_raw.rds already exists.")
    logger::log_info("\t\t Loading file from disk.")
    return(readRDS(paste0(output_path, "all_distances_raw.rds")))
  }
  ...
  all_distances_raw <- foreach(i = 1:nrow(metadata), .packages = c("GenomicRanges", "tidyverse"), .export = "getDistancesDataFrame") %dopar%{
    ...
    sample_annotated_SR_details <- annotated_SR_details %>%
      dplyr::inner_join(sample_reads, by = c("junID")) %>%
  ...
  if(rw_disk){
    logger::log_info("\t\t Saving output to ", output_path, "all_distances_raw.rds")
    all_distances_raw %>% saveRDS(paste0(output_path, "all_distances_raw.rds"))
  }
...
removeAmbiguousPairing <- function(all_distances_raw,
  ...
  ## for the results of the function in disk. If they have already been
  ## generated, the function is not executed and the files are returned.
  if (rw_disk & !overwrite & file.exists(paste0(output_path, "all_distances_pruned.rds"))) {
    logger::log_info("\t\t Ignoring ambiguous pairing removal.") 
    logger::log_info("\t\t File ", output_path, "all_distances_pruned.rds already exists.")
    logger::log_info("\t\t Loading file from disk.")
    return(readRDS(paste0(output_path, "all_distances_pruned.rds")))
  }
  ...
  if (rw_disk) {
    logger::log_info("\t\t Saving output to ", output_path, "all_distances_pruned.rds")
    all_distances_tidy %>% saveRDS(paste0(output_path, "all_distances_pruned.rds"))
  }
